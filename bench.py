...
def build_rust(path):
    ...
    bin_path = os.path.join(path, "target", "release", exe("tree_search_001"))
    return True, "ok", dur, bin_path


def build_go(path):
    if shutil.which("go") is None:
        return False, "go not found", None, None
    out = os.path.join(path, exe("app"))
    proc, dur = time_run(["go", "build", "-o", out], cwd=path)
    if proc.returncode != 0:
        return False, proc.stderr.strip(), dur, None
    return True, "ok", dur, out
...
def build_cpp(path):
    ...
    out = os.path.join(path, exe("app"))
    proc, dur = time_run(["g++", "-O2", "-std=c++17", "-o", out, "main.cpp"], cwd=path)
    if proc.returncode != 0:
        return False, proc.stderr.strip(), dur, None
    return True, "ok", dur, out
...
def run_all(problem, size, target, build=True):
    ...
    # Run via cargo to avoid guessing binary name
    if shutil.which("cargo") is not None and os.path.isdir(rust_dir):
        ...
        results.append({"lang": "rust", "ok": False, "message": "cargo or dir missing", "time": None})

    # Go
    go_dir = os.path.join(problem, "go")
    go_bin = os.path.join(go_dir, exe("app"))
    if build:
    ...
    if os.path.exists(go_bin):
        proc, rtime = time_run([go_bin, str(size), str(target)], cwd=go_dir)
    ...
    cpp_bin = os.path.join(cpp_dir, exe("app"))
    if build:
    ...
    if os.path.exists(cpp_bin):
        proc, rtime = time_run([cpp_bin, str(size), str(target)], cwd=cpp_dir)
    ...
    if os.path.exists(py_main):
        proc, rtime = time_run([py_exec, py_main, str(size), str(target)], cwd=py_dir)
